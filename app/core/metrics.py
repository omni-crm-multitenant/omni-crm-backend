    ...
    with _lock:
        return {
            "http_requests_total": {
                ...
                for (route, status), count in http_requests_total.items()
            },
            "http_request_latency_ms": {
                route: values[-1000:]
                for route, values in http_request_latency_ms.items()
            },
            "queue_depth": dict(queue_depth),
            "worker_outcomes": {
                f"{job}|{status}": count
                for (job, status), count in worker_outcomes.items()
            },
            "delivery_outcomes": {
                f"{channel}|{status}": count
                for (channel, status), count in delivery_outcomes.items()
            },
            "ai_token_usage": {
                f"{tenant}|{provider}|{model}": count
                for (tenant, provider, model), count in ai_token_usage.items()
            },
        }
